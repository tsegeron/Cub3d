 ...
 */
static void	draw_battery_charge_frame(t_map *map, int *color)
{
	int	pix_x;
	int	pix_y;
	int	pix_end_x;
	...
	while (++pix_x < pix_end_x)
	{
		...
		if (!((pix_x >= 0 && pix_x < 4)
			...
			pix_y = map->battery_bar.size_y - 4;
...
static void	draw_battery_charges(t_map *map, int *charge, int *color)
{
	...
	pix_x = 3;
	pix_end_y = map->battery_bar.size_y - 4;
	while ((*charge)--)
	{
		pix_end_x = pix_x + 26;
		while (++pix_x < pix_end_x)
...
void	draw_battery_bar(t_map *map)
{
	...
	draw_battery_charge_frame(map, &color);
